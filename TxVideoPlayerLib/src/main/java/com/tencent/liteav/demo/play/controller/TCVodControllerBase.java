...
public abstract class TCVodControllerBase extends RelativeLayout implements OnSeekBarChangeListener {
    ...
    int nowVoice;
    public void checkIsNoVoice(final ImageView mIvVoice) {
        nowVoice=getCurrentMusicVolume(getContext());
        IntentFilter filter = new IntentFilter();
        filter.addAction(AudioManager.RINGER_MODE_CHANGED_ACTION);
        filter.addAction("android.media.VOLUME_CHANGED_ACTION");
        VoiceBroadcastReceiver voiceBroadcastReceiver = new VoiceBroadcastReceiver();
        voiceBroadcastReceiver.setVoiceListener(new VoiceBroadcastReceiver.VoiceListener() {
            @Override
            public void onVoiceChange(int volume) {
                if(nowVoice-volume>0){
                    Log.i("静音", "声音变小了，当前："+volume+",原本音量："+nowVoice);
                }else if(nowVoice-volume<0){
                    Log.i("静音", "声音变大了，当前："+volume+",原本音量："+nowVoice);
                    if(!isOpenVoice){
                        isOpenVoice=true;
                        mIvVoice.setImageResource(drawable.ic_voice_open);
                        onVoiceSwitch(isOpenVoice);
                    }
                }
                nowVoice=volume;
            }
        ...
        //注册广播接收
        getContext().registerReceiver(voiceBroadcastReceiver,filter);
        AudioManager audioManager = (AudioManager) getContext().getSystemService(Context.AUDIO_SERVICE);
        if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.M) {
            boolean muteFlag = audioManager.isStreamMute(AudioManager.STREAM_MUSIC);//获取当前音乐多媒体是否静音
            isOpenVoice=!muteFlag;
            if(muteFlag){
                Log.i("Silent:", "RINGING 已被静音");
                mIvVoice.setImageResource(drawable.ic_voice_close);
            }else{
                Log.i("Silent:", "RINGING 未被静音");
                mIvVoice.setImageResource(drawable.ic_voice_open);
            }
        }
        onVoiceSwitch(isOpenVoice);
    ...
    public boolean  isAudioManagerPermissionGet(Context context, ImageView iv){
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N
            ...
            boolean muteFlag = audioManager.isStreamMute(AudioManager.STREAM_MUSIC);//获取当前音乐多媒体是否静音
            if(muteFlag){
                iv.setImageResource(drawable.ic_voice_close);
                isOpenVoice=false;
            }else{
                iv.setImageResource(drawable.ic_voice_open);
                isOpenVoice=true;
            }
            return true;
        }
    ...
    public void showSpeedPop(Activity context, TextView speedView,BeiSuPop.OnItemClickListener listener,boolean isFull){
        beiSuPop=new BeiSuPop(context);
        BeiSuPop.ViewHolder holder = beiSuPop.createPopupLayout();
        beiSuPop.showPopWindowOnTop(speedView,listener,isFull );
    }
    ...
    public void switchVoice(Context mContext,ImageView iv){
        ...
        if (isOpenVoice) {
            silentSwitchOff(mContext, iv);
        } else {
            silentSwitchOn(mContext, iv);
        }
        onVoiceSwitch(isOpenVoice);
    }

    public void onVoiceSwitch(boolean isOpenVoice){
        Log.e("静音", "执行OpenSwitch");
    }

    boolean isOpenVoice=true;
    public void silentSwitchOff(Context mContext,ImageView iv) {
        if(mContext==null){
            return;
        }
        AudioManager audioManager = (AudioManager) mContext.getSystemService(Context.AUDIO_SERVICE);
    ...
    public void silentSwitchOn(Context mContext, ImageView iv) {
        if(mContext==null){
            return;
        }
        AudioManager audioManager = (AudioManager) mContext.getSystemService(Context.AUDIO_SERVICE);
    ...
    protected void onToggleControllerView() {
        if (!this.mLockScreen) {
            if (this.isShowing) {
                ...
                this.show();
                if (this.mHideViewRunnable != null&&this.getHandler()!=null&&this.mHideViewRunnable!=null) {
                    this.getHandler().removeCallbacks(this.mHideViewRunnable);
                    this.getHandler().postDelayed(this.mHideViewRunnable, 7000L);
                }
            }
    ...
    public void setClickFullScreen(boolean clickFullScreen) {
        isClickFullScreen = clickFullScreen;
    }
